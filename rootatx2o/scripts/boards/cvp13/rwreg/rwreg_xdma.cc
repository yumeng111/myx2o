...
extern "C" void rwreg_init(char* sysfile) {
  ...
  if (strcmp("auto", sysfile) == 0) {
    char xdma_axi_lite_dev[40];
    strcpy(xdma_axi_lite_dev, "/dev/xdma0_user");
    realSysfile = xdma_axi_lite_dev;

    if (access(realSysfile, F_OK) != 0) {
      perror("ERROR: could not find the CVP13 XDMA AXI-lite device");
    }
  }
