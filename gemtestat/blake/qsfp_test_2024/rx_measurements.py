...
def gbtx_write_reg(global_gbt_idx, addr, value, rw):
    ...
    if fw_year < 0x2023:  #GE21 == GBT not GBTX
...
def gbt_config_loopback(oh, gbt, rw, x2o_gbt, x2o_utils):
    station = 2# rw.read_reg("BEFE.GEM.GEM_SYSTEM.RELEASE.GEM_STATION")
    if station == 2:
...
def measure_BER(gbt, switch, opm, rw):
    ...
    bitrate = 10.24 *10**9 #TODO: this should change depending on frontend board, 10.24 GHz is me0      
    err_measure_time = num_bits / bitrate
    
    fec_err, prbs_err = read_link_errors_gem(0, gbt, err_measure_time, rw)
...
def collect_rx_vars(chan, opm, rx_switch1, rw, x2o_gbt, x2o_utils):
    ...
    gbt = (chan-1) % 4
    oh = 3
    print(gbt_locked(oh, gbt, rw))
    # configure PRBS signal

    gbt_config_prbs(oh, gbt, rw, x2o_gbt, x2o_utils) 	
    power, avg_power, fec_err, prbs_err = measure_BER(gbt, rx_switch1, opm, rw)
