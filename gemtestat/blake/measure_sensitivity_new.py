...
# COARSE_ATT_STEP = 2.0
INITIAL_ATTENUATION_ME0 = 14
INITIAL_ATTENUATION_GE21 = 12
INITIAL_ATTENUATION_DMB = 12
INITIAL_ATTENUATION_ODMB = 10
COARSE_ATT_STEP = 0.2
FINE_ATT_STEP = 0.25
...
def configure():
    ...
    rx = conn_x2o.modules["boards.x2o.optics"]

    return power_meter, switch, voa, rw, x2o_utils, x2o_gbt, rx, x2o_fw_utils, x2o_prbs
...
def read_link_errors_csc(dmb, err_measure_time):
    if is_odmb:
        ...
        # not sure it's needed to switch PRBS off and back on, but lets start like this, and we can remove it later if needed
        links = x2o_fw_utils.befe_get_all_links()
...
def save_data(qsfp_id, source_id, oh_gbt, timestamp, att_values, power_values, fec_err_values, prbs_err_values):
    ...
    scatter_plot(power_values, fec_err_values, "power_fec_err", "Error count vs measured power (dBm)," + "{:.1e}".format(num_bits), "Measured power (dBm)", "FEC correction count", "pre-FEC errors", fig, ax1, color="b", marker="o")
...
def attenuate_and_measure(att, switch_port, bits, sleep_after_switch=1.0, use_power_meter=True, power_at_zero_att=None):
    ...
    if is_gem:
        fec_err, prbs_err = read_link_errors_gem(oh, gbt, err_measure_time)     # reads number of errors for ME0 and GE2/1
    else:
        prbs_err = read_link_errors_csc(dmb, err_measure_time)            # reads number of errors for DMB
        fec_err = prbs_err

    if use_power_meter:
        switch.output_select(SWITCH_OPM_PORT)       # selects power meter port
        power = measure_power(opm)                  # measures power
        switch.output_select(switch_port)           # selects GBT port
...
def return_error(fec_err, prbs_err):       # returns error type for testing(pre-FEC or post-FEC)
    if error_type == "fec":
        return fec_err
...
def linear_measure(left, right, step, bits, switch_port, stop_at_err=True, use_power_meter=False):          # measures between two values with a certain step size. stop_at_err is for stopping at first error found.
    print("Measuring between", round(left, 2), "and", round(right, 2), "with step", str(step) + ",", "{:.1e}".format(bits), "bits per measurement")
    att = round(left, 2)
    first_err_att = None
    while att < round(right, 2):
    ...
    if first_err_att is None:
        ...
        return first_err_att


def binary_measure(left, right, bits, switch_port, use_power_meter=False):             # not used, likely not working correctly
    att = round((left + right)/2, 2)
    power, fec_err, prbs_err = attenuate_and_measure(att, switch_port, bits, sleep_after_switch=1.0, use_power_meter=use_power_meter, power_at_zero_att=power_at_zero_att)
    fec_err_values.append(fec_err)
    prbs_err_values.append(prbs_err)
    att_values.append(att)
    ...
    bit_values.append(bits/(10**10))
    error = return_error(fec_err, prbs_err)
    if att == att_values[len(att_values) - 2]:
        return float(att)
    if error > 0:
        binary_measure(left, att, bits, switch_port, use_power_meter)
    else:
        binary_measure(att, right, bits, switch_port, use_power_meter)


def scan_att(find_edge=False, use_power_meter=True):           #entire scan procedure over all QSFP channels
    global oh
    global gbt
    global oh_gbt
    global switch_port
    ...
    for switch_oh_port in [2, 1, 0]:
        ...
        if is_gem:
            ...
            # diqable GBT header pattern search in order not to shift the MGT data when it unlocks due to switching to the OPM for measurement
        ...
        att = linear_measure(init_att, 18, COARSE_ATT_STEP, num_bits, switch_oh_port, stop_at_err=True, use_power_meter=use_power_meter)
        if find_edge:
            print("Finding the edge of failure and leaving the attenuator there")
            # att = linear_measure(att - 1, att, 0.1, 10**11, switch_oh_port, use_power_meter=use_power_meter)
            # att = linear_measure(att - 0.3, att, 0.02, 5*10**11, switch_oh_port, use_power_meter=use_power_meter)
            # att = linear_measure(att - 0.06, att, 0.02, 10**12, switch_oh_port, use_power_meter=use_power_meter)
            # print("Binary search between", att - 1, "and", att, "with", "{:.1e}".format(num_bits), "bits per measurement")
            # att = binary_measure(att - 1, att, num_bits, switch_oh_port, use_power_meter=use_power_meter)
            att -= 0.1
            print("Testing attenuation level", round(att, 2))
            power, fec_err, prbs_err = attenuate_and_measure(att, switch_oh_port, 4.75*10**12, sleep_after_switch=1.0, use_power_meter=use_power_meter, power_at_zero_att=power_at_zero_att)
            att_values.append(att)
            power_values.append(power)
            fec_err_values.append(fec_err)
            prbs_err_values.append(prbs_err)
...
if __name__ == '__main__':
    ...
    if "VTRX+" in source_id or "ME0" in source_id:
        bitrate = 10.24 * 10**9
        error_type = "fec"
        init_att = INITIAL_ATTENUATION_ME0
    elif "VTRX" in source_id or "GE21" in source_id:
        bitrate = 4.8 * 10**9
        error_type = "fec"
        init_att = INITIAL_ATTENUATION_GE21
    elif "ODMB" in source_id:
        bitrate = 12.5 * 10**9
        error_type = "prbs"
        init_att = INITIAL_ATTENUATION_ODMB
        is_odmb = True
    elif "DMB" in source_id:
        bitrate = 1.28 * 10 ** 9
        error_type = "prbs"
        init_att = INITIAL_ATTENUATION_DMB
    else:
    ...
    opm, switch, voa, rw, x2o_utils, x2o_gbt, rx, x2o_fw_utils, x2o_prbs = configure()
    ...
    qsfps = rx.x2o_get_qsfps()

    disable_rx_squelch = True
    enable_rx_squelch = None

    if disable_rx_squelch is not None:
        cage = QSFP_CAGE
        if cage not in qsfps:
            rx.print_red("Cannot disable RX squelch on cage %d, because there's no QSFP installed in that cage" % cage)
        else:
            print("Disabling RX squelch on cage %d" % cage)
            rx.x2o_disable_rx_squelch(qsfps[cage])
            time.sleep(1)

    if enable_rx_squelch is not None:
        cage = QSFP_CAGE
        if cage not in qsfps:
            rx.print_red("Cannot enable RX squelch on cage %d, because there's no QSFP installed in that cage" % cage)
        else:
            print("Enabling RX squelch on cage %d" % cage)
            rx.x2o_enable_rx_squelch(qsfps[cage])
            time.sleep(1)
